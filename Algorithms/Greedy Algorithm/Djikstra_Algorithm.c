...
// Function that implements Djikstra's single source shortest path algorithm for a graph represented usingadjacency matrix representation
void dijkstra(int graph[V][V], int src) {
    int dist[V]; // The output array. dist[i] will hold the shortest distance from src to i
    int sptSet[V]; //sptSet[i] will be true if the vertex i is included in the shortest path tree or shortest distance from src to i is finalized
    ...
    for (int count = 0; count<V-1; count++) {
        //Pick the minimum distance vertex from teh set of vertices not yet processed
